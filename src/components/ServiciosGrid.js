...
export default function ServiciosGrid({ servicios, onEditar, onCambiarEstado, soloLectura }) {
          ...
          <div className="mt-4 pt-4 border-t border-slate-100 flex items-center justify-between">
            ...
            {!soloLectura && (
              <div className="flex gap-3 opacity-0 group-hover:opacity-100 transition">
                ...
                <button
                  onClick={() => onCambiarEstado(s)}
                  aria-label={s.activo ? "Desactivar servicio" : "Activar servicio"}
                  className={s.activo ? "text-slate-400 hover:text-rose-600" : "text-slate-400 hover:text-emerald-600"}
                >
